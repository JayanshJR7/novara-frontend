...
//         if (verificationResponse.testMode) {
//           console.log('⚠️ TEST MODE: Payment auto-approved (Admin user)');
//         }
...
//               paymentSuccess: true,
//               orderId: orderId,
//               testMode: verificationResponse.testMode || false
//             }
//           });
//         }, 2000);
...
const Checkout = () => {
  ...
  // ✅ FIXED: Admin Direct Order Placement (No Payment)
  const placeAdminTestOrder = async (orderData) => {
    try {
      setProcessingPayment(true);
      setError('');

      console.log('🔧 Admin Test Mode: Creating order without payment...');
      ...
      const verificationData = {
        razorpay_order_id: 'test_admin_order_' + Date.now(),
        razorpay_payment_id: 'test_admin_payment_' + Date.now(),
        razorpay_signature: 'test_admin_signature',
        orderId: order._id
      };

      console.log('🔧 Verifying admin test order...');

      const verificationResponse = await paymentAPI.verifyPayment(verificationData);

      if (verificationResponse.success) {
        console.log('✅ Admin test order verified!');
        ...
        throw new Error('Admin test order verification failed');
      }

    } catch (err) {
      console.error('❌ Admin test order error:', err);
      setError(err.response?.data?.message || err.message || 'Failed to place admin test order.');
      setProcessingPayment(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ FIXED: Added user field
      const orderData = {
        user: user._id, // ← ADDED THIS
        customerName: formData.customerName,
        email: formData.email,
        phone: formData.phone,
        shippingAddress: {
      ...
      // ✅ CHECK IF ADMIN - If yes, skip Razorpay modal
      if (user && user.isAdmin) {
        console.log('🔧 ADMIN USER DETECTED - Skipping Razorpay payment modal');
        setLoading(false);
        await placeAdminTestOrder(orderData);
        return;
      }

      // ✅ NORMAL USER FLOW - Open Razorpay modal
      console.log('👤 Regular user - Opening Razorpay payment modal');
          ...
          <div className="processing-payment">
            {!paymentVerified ? (
              <>
                ...
                <h2>
                  {user && user.isAdmin 
                    ? '🔧 Processing Admin Test Order...' 
                    : 'Verifying Payment...'}
                </h2>
                <p>
                  {user && user.isAdmin
                    ? 'Creating test order without payment (Admin mode)'
  ...
  return (
    <div className="checkout-page">
      <div className="container">
        ...
        {user && user.isAdmin && (
          <div className="admin-test-mode-banner" style={{
            ...
            <p style={{ margin: 0, color: '#856404', fontSize: '14px' }}>
              You won't be charged. Order will be created automatically for testing.
            </p>
          </div>
        )}
        ...
        <div className="checkout-layout">
          ...
          <div className="checkout-form-section">
            ...
            <form onSubmit={handleSubmit} className="checkout-form">
              ...
              <div className="form-section">
                ...
                <div className="payment-info-box">
                  {user && user.isAdmin ? (
                    <>
                      <p className="payment-method-selected">
                        <strong className='color-primary'>🔧 Admin Test Mode - No Payment Required</strong>
                      </p>
                      <p className="payment-description">
                        Order will be created automatically for testing purposes
                      </p>
                      <ul className="payment-features">
                        <li>✓ No actual payment needed</li>
                        <li>✓ Order auto-confirmed</li>
                        <li>✓ Telegram notification sent</li>
                        <li>✓ Test mode clearly marked</li>
                      </ul>
                    </>
                  ) : (
                    <>
              ...
              >
                {loading 
                  ? (user && user.isAdmin ? 'Creating Test Order...' : 'Initiating Payment...') 
                  : (user && user.isAdmin ? '🔧 Place Test Order (Free)' : 'Proceed to Payment')
                }
              </button>
            ...
            <div className="order-summary-card">
              ...
              <div className="summary-items">
                {checkoutItems.map((item) => {
                  ...
                  const productImage = item.product.itemImages && item.product.itemImages.length > 0
                    ? item.product.itemImages[0]
                    : (item.product.itemImage || '/placeholder.png');
                  
                  const productName = item.product.itemName || item.product.itemname || 'Unnamed Product';
              ...
              {user && user.isAdmin && (
                <p style={{ 
                  ...
                  🔧 Test Order - No Payment Required
                </p>
              )}
            </div>
          </div>
